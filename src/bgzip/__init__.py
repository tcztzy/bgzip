...
from typing import Generator, IO, List, Sequence, Tuple, TypedDict

from bgzip import utils as utils  # type: ignore
...
class InflateInfo(TypedDict):
    blocks: List[memoryview]
    bytes_inflated: int
    bytes_read: int
    remaining_chunks: List[memoryview]
...
class BGZipReader(io.RawIOBase):
    ...
    def _fetch_and_inflate(self):
        while True:
            ...
            inflate_info = utils.inflate_chunks(
                [memoryview(self._input_data)],
            ...
            if self._input_data and not inflate_info["bytes_inflated"]:
                ...
                ), "Read error. Please contact bgzip maintainers."
                self._start = self._stop = 0
            else:
...
def inflate_chunks(
    ...
    inflate_info = utils.inflate_chunks(chunks, inflate_buf, num_threads, atomic=atomic)
    blocks: List[memoryview] = []
    total = 0
    for sz in inflate_info["block_sizes"]:
        blocks.append(inflate_buf[total : total + sz])
        total += sz
...
class BGZipWriter(io.IOBase):
    ...
    def _compress(self, process_all_chunks=False):
        while self._input_buffer:
            ...
            if (
                len(self._input_buffer) < utils.block_data_inflated_size
                and not process_all_chunks
            ):
                break
    ...
    def write(self, data):
        ...
        if (
            ...
            > utils.block_batch_size * utils.block_data_inflated_size
        ):
            self._compress()
...
class Deflater:
    def __init__(
        self,
        num_threads: int = cpu_count(),
        num_deflate_buffers: int = utils.block_batch_size,
    ):
    ...
    @staticmethod
    def _gen_buffers(
        number_of_buffers: int = utils.block_batch_size,
    ) -> List[bytearray]:
        if 0 >= number_of_buffers or utils.block_batch_size < number_of_buffers:
            raise ValueError(f"0 < 'number_of_buffers' <= '{utils.block_batch_size}")
        # Include a kilobyte of padding for poorly compressible data
        max_deflated_block_size = (
            utils.block_data_inflated_size + utils.block_metadata_size + 1024
        )
    ...
    def deflate(self, data: memoryview) -> Tuple[int, List[memoryview]]:
        deflated_sizes = utils.deflate_to_buffers(
            data, self._deflate_bufs, self._num_threads
        )
        bytes_deflated = min(
            len(data), utils.block_data_inflated_size * len(deflated_sizes)
        )
        return bytes_deflated, [
            memoryview(buf)[:sz] for buf, sz in zip(self._deflate_bufs, deflated_sizes)
        ]
